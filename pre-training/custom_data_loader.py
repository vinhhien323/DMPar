...
def custom_collate(data, tokenizer, device = None):
    ...
    all_head_idx = torch.stack([item[3] for item in data])
    all_label_ids = torch.stack([item[4] for item in data])
    all_valid_ids = torch.stack([item[5] for item in data])
    all_lmask_ids = torch.stack([item[6] for item in data])
    all_eval_mask_ids = torch.stack([item[7] for item in data])
